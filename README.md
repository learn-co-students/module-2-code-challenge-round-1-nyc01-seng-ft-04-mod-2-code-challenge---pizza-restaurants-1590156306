...
X A Pizza can be sold at many Restaurants
X A Restaurant can sell many Pizzas
X RestaurantPizza stores an integer price for each pizza (Remember, each Restaurant can choose how to price its own Pizzas)
...
X Add a validation to RestaurantPizza to ensure that each Pizza can only appear once on the same Restaurant's menu.
X Update the error handling on the RestaurantPizza creation form to show this validation error.
